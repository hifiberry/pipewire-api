...
pub fn create_router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route("/api/v1/io", get(get_io))
        .route("/api/v1/status", get(get_status))
        .route("/api/v1/eq/:block/:band", get(get_eq_band).put(set_eq_band))
        .route("/api/v1/eq/:block/:band/enabled", put(set_eq_band_enabled))
        .route("/api/v1/eq/:block/clear", put(clear_eq_bank))
        .route("/api/v1/gain/master", get(get_master_gain).put(set_master_gain))
        .route("/api/v1/enable", get(get_enable).put(set_enable))
        .with_state(state)
}
